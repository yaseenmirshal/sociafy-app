  ...
  return (
    <div>
      ...
      {post && post.map((item: any, index: any) => (
        <div key={index} style={{ borderRadius: '20px', width: '620px', height: '590px', backgroundColor: isWhite ? 'black' : '#DEDEDE', color: isWhite ? 'white' : 'black' }} className='ml-12 mt-8'>
          ...
          <div className='w-36 h-5'><p style={{ padding: '25px', fontSize: '15px' }} className='ml-12'>{localStorage.getItem("username")}</p></div>
          <Delete item={item._id}/>

          <div style={{ borderRadius: '20px', width: '570px', height: '400px', backgroundImage: `url(${item.image})`, backgroundSize: 'cover', backgroundPosition: 'center', marginTop: '35px', marginLeft: '25px' }}>
          </div>
         
          <div className= 'overflow-scroll w-[550px] h-4 ml-7 mt-2 '>
            <p className='text-sm'>{localStorage.getItem("username")}. {item.desc}</p>
          </div>
          
          {/* <div className='overflow-scroll w-[550px] h-6 ml-7 bg-red-400'>
           
          </div> */}
          <div className='mt-2'>
            <svg onClick={()=>fetchLike(item._id)} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="float-left w-8 h-8 mr-1 ml-6 cursor-pointer">
            ...
            <h1 style={{ paddingTop: '5px' }} className='float-left'>0</h1>
      ...
      />
      <button onClick={()=>fetchComment(item._id)} className="px-4 py-2 rounded-r-md bg-blue-500 text-white hover:bg-blue-600 focus:outline-none">
        Submit
      </button>
    </div>
    {/* <h1>All comments :{item.comments}</h1> */}
          </div>
        </div>
      ))}
    </div>
